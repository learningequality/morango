...
class MakeDeferrableForeignKeysTestCase(TransactionTestCase):
    ...
    @pytest.mark.skipif(getattr(settings, "MORANGO_TEST_POSTGRESQL", False), reason="SQLite only")
    ...
    def test_iter_apps__all(self):
        ...
        self.assertEqual(
            [app.name for app in op._iter_apps(global_apps)],
            [*BASE_APPS, "morango", "facility_profile"],
        )
