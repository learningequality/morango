...
def _insert_model_into_profile_dict(model, profile):
    ...
    class_indices = [_profiles_for_syncing_models[profile].index(cls) for cls in foreign_key_classes if cls in _profiles_for_syncing_models[profile]]
    ...
    _profiles_for_syncing_models[profile].insert(insert_after_idx, model)
...
def add_syncing_models():
    ...
    # if this is not false, it means this dict has been populated, so we exit
    if _profiles_for_syncing_models:
        return
    ...
    for model_class in django.apps.apps.get_models():
        ...
        if issubclass(model_class, SyncableModel) and not model_class._meta.proxy:
            ...
            if not hasattr(model_class, 'morango_model_name'):
                raise InvalidMorangoModelConfiguration("{} model must define a morango_model_name".format(name))

            # create empty list to hold model classes for profile if not yet created
            profile = model_class._morango_profile
            _profiles_for_syncing_models[profile] = _profiles_for_syncing_models.get(profile, [])
    ...
    for profile, model_list in iteritems(_profiles_for_syncing_models):
        _syncing_models_dict = OrderedDict()
        for model_class in model_list:
            _syncing_models_dict[model_class.morango_model_name] = model_class
        _profiles_for_syncing_models[profile] = _syncing_models_dict
