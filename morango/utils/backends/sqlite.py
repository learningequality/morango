...
class SQLWrapper(BaseSQLWrapper):
    ...
    def _bulk_insert_into_app_models(self, cursor, app_model, fields, db_values, placeholder_list):
        ...
        value_chunks = [db_values[x:x + num_of_values_able_to_insert] for x in xrange(0, len(db_values), num_of_values_able_to_insert)]
        placeholder_chunks = [placeholder_list[x: x + num_of_rows_able_to_insert] for x in xrange(0, len(placeholder_list), num_of_rows_able_to_insert)]
        # insert data chunks
    ...
    def _dequeuing_merge_conflict_buffer(self, cursor, current_id, transfersession_id):
        ...
        merge_conflict_store = """REPLACE INTO {store} (id, serialized, deleted, last_saved_instance, last_saved_counter, hard_deleted, model_name,
    ...
    def _dequeuing_insert_remaining_buffer(self, cursor, transfersession_id):
        ...
        insert_remaining_buffer = """REPLACE INTO {store} (id, serialized, deleted, last_saved_instance, last_saved_counter, hard_deleted,
