...
class SQLWrapper(BaseSQLWrapper):
    ...
    def _dequeuing_merge_conflict_buffer(self, cursor, current_id, transfersession_id):
        ...
        merge_conflict_store = """UPDATE {store} as store SET (serialized, deleted, last_saved_instance, last_saved_counter, hard_delete, model_name,
                                                        profile, partition, source_id, conflicting_serialized_data, dirty_bit, _self_ref_fk)
                                            = (CASE buffer.hard_delete WHEN TRUE THEN '' ELSE store.serialized END, store.deleted OR buffer.deleted, '{current_instance_id}',
                                                   {current_instance_counter}, store.hard_delete, store.model_name, store.profile, store.partition, store.source_id,
                                                   CASE buffer.hard_delete WHEN TRUE THEN '' ELSE buffer.serialized || '\n' || store.conflicting_serialized_data END, TRUE, store._self_ref_fk)
                                            /*Scope to a single record.*/
    ...
    def _dequeuing_insert_remaining_buffer(self, cursor, transfersession_id):
        ...
        insert_remaining_buffer = """
            ...
            updated as
            (
                UPDATE {store} store SET (serialized, deleted, last_saved_instance, last_saved_counter, hard_delete,
                                     model_name, profile, partition, source_id, conflicting_serialized_data, dirty_bit, _self_ref_fk)
                                    = (nv.serialized, nv.deleted, nv.last_saved_instance, nv.last_saved_counter, nv.hard_delete,
            ...
            INSERT INTO {store}(id, serialized, deleted, last_saved_instance, last_saved_counter, hard_delete,
                                model_name, profile, partition, source_id, conflicting_serialized_data, dirty_bit, _self_ref_fk)
            SELECT ut.model_uuid, ut.serialized, ut.deleted, ut.last_saved_instance, ut.last_saved_counter, ut.hard_delete,
